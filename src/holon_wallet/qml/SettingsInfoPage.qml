...
PageState {
    ...
    function bodyText() {
        if (section === "network")
            return "Ethereum, Base, Arbitrum One, and OP Mainnet balances are read through approved public RPC endpoints. ETH/USD and USDC/USD use fixed Chainlink feeds on Base. Network data can be unavailable and is never used to authorize a transaction."
        if (section === "security")
            return "Accounts are stored in an encrypted local vault. Every mainnet transfer or revoke requires a fresh password and a click on its exact action button. Authorization is valid for one exact action only."

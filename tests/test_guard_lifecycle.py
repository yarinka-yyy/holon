...
from guard_support import ACTION_ID, ACTION_ID_2, FINGERPRINT, make_ledger
class FakeHandle:
...
class GuardLifecycleTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.ledger = make_ledger(Path(self.temporary.name))
        identifiers = iter(("flow-one", "flow-two"))
        self.guard = GuardLifecycle(
            self.store, self.store.load(), self.wallet, self.owner, self.ledger,
            id_factory=lambda: next(identifiers),
            clock=lambda: 2.0,
        )

    def test_start_persists_entering_then_enforces_one_active_flow(self) -> None:
        started = self.guard.start_flow(101, ACTION_ID, FINGERPRINT)
        ...
        self.assertEqual(
            self.guard.start_flow(101, ACTION_ID_2, "b" * 64).code,
    ...
    def test_matching_cancel_closes_cleanly_and_wrong_id_is_rejected(self) -> None:
        ...
        self.assertEqual(self.guard.cancel_flow(ACTION_ID_2).code, "ACTION_ID_MISMATCH")
        closing = self.guard.cancel_flow(ACTION_ID)
    ...
    def test_wallet_or_owner_loss_requires_matching_recovery_and_new_id(self) -> None:
        ...
        self.assertEqual(self.guard.recover_flow(ACTION_ID_2).code, "ACTION_ID_MISMATCH")
        self.assertTrue(self.guard.recover_flow(ACTION_ID).ok)
        self.wallet.handle = FakeHandle(303)
        restarted = self.guard.start_flow(101, ACTION_ID_2, "b" * 64)
    ...
    def test_launch_and_close_failures_require_recovery(self) -> None:
        ...
        result = self.guard.start_flow(101, ACTION_ID, FINGERPRINT)
        self.assertEqual(result.code, "WALLET_LAUNCH_FAILED")
        self.assertIs(self.guard.snapshot.state, GuardState.RECOVERY_REQUIRED)
        self.assertEqual(
            self.guard.start_flow(101, ACTION_ID_2, "b" * 64).code,
            "RECOVERY_REQUIRED",
        )
        self.guard.recover_flow(ACTION_ID)
        self.wallet.launch_error = False
        self.wallet.close_error = True
        self.wallet.handle = FakeHandle(404)
        self.guard.start_flow(101, ACTION_ID_2, "b" * 64)
        self.assertEqual(self.guard.cancel_flow(ACTION_ID_2).code, "WALLET_INTERRUPTED")
    ...
    def test_signing_disabled_refuses_authority_commands(self) -> None:
        ...
        guard = GuardLifecycle(self.store, disabled, self.wallet, self.owner, self.ledger)
        self.assertEqual(guard.start_flow(101, ACTION_ID, FINGERPRINT).code, "SIGNING_DISABLED")
        self.assertEqual(guard.cancel_flow(ACTION_ID).code, "FLOW_NOT_ACTIVE")
        self.assertEqual(guard.recover_flow(ACTION_ID).code, "RECOVERY_NOT_REQUIRED")
        self.assertTrue(guard.health().ok)

    def test_unavailable_owner_never_launches_wallet(self) -> None:
        self.owner.alive = False
        result = self.guard.start_flow(101, ACTION_ID, FINGERPRINT)
        self.assertEqual(result.code, "OWNER_UNAVAILABLE")
